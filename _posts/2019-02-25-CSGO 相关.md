...
bind "\" "toggle bot_mimic 1 2"	//调整Bot视角
...
综上，首先放置一个bot到你差不多想放的位置上(bot_place)，然后开bot_mimic 1 和 bot_mimic_yaw_offset 0 将bot放到精确位置并调整好视角，最后输入bot_mimic 2。
...
### net_graph 0/1
显示帧数 fps, 延迟 ping 和丢包 loss 等参数， 个人绑定在 `TAB` 上，按下时显示网络参数和记分板，松开时关闭
~~~
